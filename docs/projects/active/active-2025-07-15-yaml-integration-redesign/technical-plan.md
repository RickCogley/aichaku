...
function findYamlBlock(content: string): { start: number; end: number } | null {
  ...
  const endIdx = content.indexOf(YAML_BLOCK_END, startIdx + YAML_BLOCK_START.length);
...
function replaceYamlBlock(content: string, yamlConfig: AichakuYamlConfig): string {
...
```typescript
async function loadStandardMetadata(standardId: string): Promise<StandardMetadata> {
...
function extractCustomContent(content: string): string {
  ...
  const markers = [
    ...
    { start: "<!-- AICHAKU:METHODOLOGY:START -->", end: "<!-- AICHAKU:METHODOLOGY:END -->" },
    { start: "<!-- AICHAKU:STANDARDS:START -->", end: "<!-- AICHAKU:STANDARDS:END -->" },
    { start: "<!-- AICHAKU:DOC-STANDARDS:START -->", end: "<!-- AICHAKU:DOC-STANDARDS:END -->" }
  ];
...
**Design:** The learn command will read YAML metadata to dynamically generate its content without duplicating information.
...
- Size: ${formatBytes(before)} → ${formatBytes(after)} (${Math.round((1 - after/before) * 100)}% reduction)
