...
import type { ClaudeGuidance, ReviewResult, Finding } from "./types.ts";
...
export class FeedbackBuilder {
  ...
  private buildSpecificGuidance(finding: Finding, result: ReviewResult): ClaudeGuidance {
  ...
  private buildCommandInjectionGuidance(finding: Finding, result: ReviewResult): ClaudeGuidance {
    const count = result.findings.filter(f => f.rule === finding.rule).length;
    
    return {
      reminder: `Your CLAUDE.md security standards require preventing command injection, but you have ${count} instance${count > 1 ? 's' : ''}.`,
      ...
      context: "Command injection is one of the most critical security vulnerabilities. Attackers can execute arbitrary commands by manipulating input.",
...
exec('ls', [userPath])`,
      ...
      reinforcement: "Always use parameter expansion or array arguments for shell commands. This is a critical security requirement.",
    };
  }

  private buildTypeScriptAnyGuidance(finding: Finding, result: ReviewResult): ClaudeGuidance {
    const count = result.findings.filter(f => f.rule === finding.rule).length;
    
    return {
      reminder: `Your CLAUDE.md explicitly states to avoid 'any' types, but you used it ${count} time${count > 1 ? 's' : ''}.`,
      ...
      context: "TypeScript's type system prevents runtime errors and makes code self-documenting. Using 'any' disables these benefits.",
...
if (isResponseData(data)) {
  // Now data is typed
      ...
      reflection: "What prevented you from defining a proper type? Was it time pressure or uncertainty about the structure?",
      
      reinforcement: "TypeScript is most valuable when we use its type system. Take the time to define proper types.",
    };
  }

  private buildPathTraversalGuidance(finding: Finding, result: ReviewResult): ClaudeGuidance {
    return {
      ...
      context: "Attackers use '../' sequences to access files outside the intended directory, potentially reading sensitive configuration or system files.",
...
const content = await Deno.readTextFile(resolved);`,
      ...
      reflection: "Did you consider what would happen if a user provided '../../../etc/passwd' as input?",
      
      reinforcement: "Always validate paths to ensure they stay within intended boundaries. This is an OWASP A01 vulnerability.",
    };
  }

  private buildAccessControlGuidance(finding: Finding, result: ReviewResult): ClaudeGuidance {
    return {
      ...
      context: "Without proper access control, users can access resources they shouldn't, leading to data breaches and privilege escalation.",
...
app.get('/api/users/:id', 
  ...
  async (req, res) => {
    ...
    res.json(user);
      ...
      reinforcement: "Every resource access must verify both authentication (who you are) and authorization (what you can do).",
    };
  }

  private build15FactorConfigGuidance(finding: Finding, result: ReviewResult): ClaudeGuidance {
    return {
      ...
      context: "Hardcoded values make it impossible to run the same code in different environments (dev, staging, production) without modification.",
...
if (!API_URL) {
  throw new Error("API_URL environment variable is required");
      ...
      reinforcement: "Configuration belongs in the environment, not in code. This enables the same code to run anywhere.",
    };
  }

  private buildGenericGuidance(finding: Finding, result: ReviewResult): ClaudeGuidance {
    const count = result.findings.filter(f => f.rule === finding.rule).length;
    
    return {
      reminder: `Found ${count} instance${count > 1 ? 's' : ''} of ${finding.rule} violations.`,
      pattern: finding.message,
      correction: finding.suggestion || "Follow the security and coding standards",
      
      context: `This issue has ${finding.severity} severity and should be addressed to maintain code quality and security.`,
